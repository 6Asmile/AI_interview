...
import { getPostListApi, getCategoryListApi, getTagListApi, type PostListItem, type Category, type Tag } from '@/api/modules/blog';
...
const activeFilter = ref<{ type: 'category' | 'tag' | null, slug: string | null }>({
  type: null,
  slug: null,
});
...
const fetchPosts = async () => {
  ...
  try {
    ...
    if (activeFilter.value.type === 'category' && activeFilter.value.slug) {
      params.category__slug = activeFilter.value.slug;
    } else if (activeFilter.value.type === 'tag' && activeFilter.value.slug) {
      params.tags__slug = activeFilter.value.slug;
    }
...
const fetchSidebarData = async () => {
  ...
  try {
    const [catResponse, tagResponse, hotPostResponse] = await Promise.all([
      getCategoryListApi(),
      getTagListApi(),
      getPostListApi({ ordering: '-view_count', page_size: 5 })
    ]);
    categories.value = catResponse.results;
    tags.value = tagResponse.results;
...
const handleFilterChange = (type: 'category' | 'tag' | null, slug: string | null) => {
  if (activeFilter.value.type === type && activeFilter.value.slug === slug) {
    activeFilter.value = { type: null, slug: null };
  } else {
    activeFilter.value = { type, slug };
  }
  currentPage.value = 1;
  fetchPosts();
};
    ...
    <el-row :gutter="20">
        ...
        <div class="sticky-sidebar left-sidebar">
          <div class="sidebar-module">
            <ul class="category-nav-list">
              <li :class="{ active: activeFilter.type === null }" @click="handleFilterChange(null, null)">
                <el-icon><CollectionTag /></el-icon> 综合
              </li>
              <li
                v-for="cat in categories"
                :key="cat.id"
                :class="{ active: activeFilter.type === 'category' && activeFilter.slug === cat.slug }"
                @click="handleFilterChange('category', cat.slug)">
                {{ cat.name }}
              </li>
            </ul>
          </div>
      ...
      <el-col :xs="24" :sm="6" :md="6">
        <div class="sticky-sidebar right-sidebar">
          <template v-if="!isSidebarLoading">
            <div class="sidebar-module">
             ...
             <div class="sidebar-module">
              ...
              <div class="tag-cloud">
                 <el-tag 
                    ...
                    :key="tag.id" 
                    class="tag-item" 
                    :effect="activeFilter.type === 'tag' && activeFilter.slug === tag.slug ? 'dark' : 'plain'"
                    round
                    @click="handleFilterChange('tag', tag.slug)"
                  >
                    {{ tag.name }}
                  </el-tag>
              </div>
...
.posts-list .post-card-link:last-child .post-card { 
  border-bottom: none; 
}
